...
const App: FC = () => {
    ...
    const spsRoute = async (
        ...
        const routeProfile = 'skolerute'
        const school = skoleData.find((item) => item.id === schoolId);
        const url = `https://kort.lolland.dk/spsroute/api/1.0/route?profile=${routeProfile}&from=${school.latLong}&to=${toCoord}&srs=epsg:25832&lang=da`;
